...
@section('title', '| Registrasi Satuan')
@section('breadcrumb', 'Daftarkan Satuan Baru')
    ...
    <div class="space-y-8 animate-fade-in-down">
        ...
        <div class="flex items-center space-x-5">
            ...
            <div>
                ...
                <h2 class="text-3xl font-bold text-primary tracking-tight">Daftarkan Kesatuan Baru</h2>
            </div>
        </div>

        <div class="max-w-3xl">
            <form action="{{ route('satuan.store') }}" method="POST" class="space-y-8">
                @csrf
                ...
                <div class="layout-card overflow-hidden">
                    <div class="bg-muted/30 p-6 border-b border-border">
                        <h3 class="text-[10px] font-bold text-primary uppercase tracking-[0.3em] flex items-center">
                            <i class="fas fa-file-contract mr-3 opacity-30"></i> Rincian Kesatuan
                        </h3>
                    </div>
                    ...
                    <div class="p-8 space-y-8">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="text" name="kode_satuan" id="kode_satuan"
                                        ...
                                        placeholder="e.g. SATKATPUR" value="{{ old('kode_satuan') }}" required>
                                </div>
                                @error('kode_satuan')
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="text" name="nama_satuan" id="nama_satuan"
                                        class="form-input pl-10 h-12 bg-background border-border hover:border-primary/30 transition-all text-xs font-bold uppercase tracking-widest placeholder:text-muted-foreground/30"
                                        placeholder="e.g. SATUAN KAPAL CEPAT" value="{{ old('nama_satuan') }}" required>
                                </div>
                                @error('nama_satuan')
                                    <p class="text-destructive text-[10px] font-bold mt-2 uppercase tracking-tighter italic">
                                        {{ $message }}</p>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="4"
                                ...
                                placeholder="Tentukan lingkup operasional dan peran strategis kesatuan ini...">{{ old('deskripsi') }}</textarea>
                            @error('deskripsi')
                                <p class="text-destructive text-[10px] font-bold mt-2 uppercase tracking-tighter italic">
                                    {{ $message }}</p>
                            @enderror
                    ...
                    <div class="p-8 bg-muted/30 border-t border-border flex items-center justify-end gap-4">
                        <a href="{{ route('satuan.index') }}"
                            class="px-6 py-2.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest hover:text-primary transition-colors">Batalkan</a>
                        <button type="submit"
                            class="btn-primary h-12 px-10 rounded-lg shadow-xl shadow-primary/20 active:scale-[0.98] transition-all">
                            Simpan Kesatuan <i class="fas fa-save ml-3"></i>
                        </button>
                    </div>
                </div>
            </form>
