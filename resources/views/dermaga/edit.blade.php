...
@section('title', '| Edit Dermaga')
@section('breadcrumb', 'Edit Dermaga')
...
@section('content')
    <div class="mb-8 flex items-center space-x-4 animate-fade-in">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Edit Data Dermaga</h2>
            <p class="text-gray-500 font-medium italic">Perbarui rincian infrastruktur: <span
                    class="text-navy-primary font-bold">{{ $dermaga->nama_dermaga }}</span></p>
        </div>
    </div>

    <form action="{{ route('dermaga.update', $dermaga->id) }}" method="POST" class="max-w-5xl mx-auto space-y-6 pb-20">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        <div class="md:col-span-2 space-y-2">
                            ...
                            <input type="text" name="nama_dermaga" id="nama_dermaga" required class="form-input"
                                placeholder="Misal: Dermaga Madura Barat"
                        ...
                        <div class="space-y-2">
                            <label for="kode_dermaga"
                                ...
                                <span class="text-navy-primary font-black italic text-[9px]">UNIQUE</span>
                            </label>
                            <input type="text" name="kode_dermaga" id="kode_dermaga" required class="form-input"
                                placeholder="DMB-01" value="{{ old('kode_dermaga', $dermaga->kode_dermaga) }}">
                            @error('kode_dermaga')
                                <p class="text-red-500 text-[10px] mt-2 font-bold italic">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="tipe_dermaga" id="tipe_dermaga" required class="form-input cursor-pointer">
                                @foreach ($tipeOptions as $option)
                                    <option value="{{ $option }}"
                                        {{ old('tipe_dermaga', $dermaga->tipe_dermaga) == $option ? 'selected' : '' }}>
                                        {{ $option }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="number" step="0.1" name="panjang_m" id="panjang_m" required
                                    class="form-input pr-10" placeholder="0.0"
                                    value="{{ old('panjang_m', $dermaga->panjang_m) }}">
                                <span
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="number" step="0.1" name="lebar_m" id="lebar_m" required
                                    class="form-input pr-10" placeholder="0.0"
                                    value="{{ old('lebar_m', $dermaga->lebar_m) }}">
                                <span
                                    class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300 text-[10px] font-bold">M</span>
                            </div>
                        </div>
                        <div class="space-y-2">
                            ...
                            <div class="relative font-mono">
                                <input type="number" step="0.1" name="kedalaman_min_lws" id="kedalaman_min_lws"
                                    ...
                                    value="{{ old('kedalaman_min_lws', $dermaga->kedalaman_min_lws) }}">
                                <span
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative font-mono">
                                <input type="number" step="0.1" name="kedalaman_max_lws" id="kedalaman_max_lws"
                                    required class="form-input pr-10 text-blue-600 font-bold" placeholder="0.0"
                                    value="{{ old('kedalaman_max_lws', $dermaga->kedalaman_max_lws) }}">
                                <span
                                    class="absolute right-4 top-1/2 -translate-y-1/2 text-blue-200 text-[10px] font-bold">LWS</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <input type="text" name="latitude" id="latitude" class="form-input font-mono"
                                placeholder="-7.206584" value="{{ old('latitude', $dermaga->latitude) }}">
                        </div>
                        <div class="space-y-2 md:col-span-2">
                            <label for="longitude"
                                class="text-[11px] font-bold text-gray-400 uppercase tracking-widest flex items-center">
                            ...
                            <input type="text" name="longitude" id="longitude" class="form-input font-mono"
                                placeholder="112.730302" value="{{ old('longitude', $dermaga->longitude) }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="layout-card p-8">
                    ...
                    <textarea name="catatan_operasional" id="catatan_operasional" rows="4" class="form-input resize-none"
                        placeholder="Tuliskan catatan khusus atau instruksi operasional untuk dermaga ini...">{{ old('catatan_operasional', $dermaga->catatan_operasional) }}</textarea>
                </div>
            </div>
            ...
            <div class="lg:col-span-4 space-y-8">
                        ...
                        @foreach ($statusOptions as $option)
                            <label
                                ...
                                <input type="radio" name="status" value="{{ $option }}"
                                    ...
                                    {{ old('status', $dermaga->status) == $option ? 'checked' : '' }}>
                                <span
                            ...
                            <label class="flex items-center px-2 py-1 group cursor-pointer">
                                <div class="relative flex items-center">
                                    <input type="checkbox" name="fasilitas[]" value="{{ $fasilitas }}"
                                        ...
                                        {{ in_array($fasilitas, old('fasilitas', $dermaga->fasilitas ?? [])) ? 'checked' : '' }}>
                                </div>
                                <span
                ...
                <!-- Action Card -->
                <div class="layout-card p-6 bg-blue-600 border-none shadow-xl shadow-blue-600/30">
                    <button type="submit"
                        class="w-full h-14 bg-white text-blue-600 font-black text-xs uppercase tracking-[0.2em] rounded-xl hover:-translate-y-1 hover:shadow-2xl transition-all active:scale-95">
                        Simpan Perubahan <i class="fas fa-sync ml-3"></i>
                    </button>
                    <a href="{{ route('dermaga.index') }}"
